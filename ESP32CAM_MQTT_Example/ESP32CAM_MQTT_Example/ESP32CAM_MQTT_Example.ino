...
//Datos de WiFi
const char* ssid = "Ocorp";  // Aquí debes poner el nombre de tu red
const char* password = "Ccj9xqiz29";  // Aquí debes poner la contraseña de tu red
...
void loop(){
  ...
  if (timeNow - timeLast > wait) { // Manda un mensaje por MQTT cada cinco segundos
    ...
    client.publish("esp32g4/data", dataString); // Esta es la función que envía los datos por MQTT, especifica el tema y el valor
...
void callback(char* topic, byte* message, unsigned int length) {
  ...
  if (String(topic) == "esp32g4/output") {  // En caso de recibirse mensaje en el tema esp32/output
    if(messageTemp == "true"){
...
void reconnect(){
  ...
  while (!client.connected()){ // Pregunta si hay conexión
    ...
    // Intentar reconexión
    if (client.connect("OctavioRamirezIoT23092021")) { //Pregunta por el resultado del intento de conexión
      Serial.println("Conectado");
      client.subscribe("esp32g4/output"); // Esta función realiza la suscripción al tema
